...
# Purpose:
#
# The point is to collect kube config files in a known directory, e.g. $HOME/.kube/configd.
...
# An input config file, e.g. $HOME/dev/ckube/admin.conf, generated by kubeadm
# A destination location, e.g. $HOME/.kube/configd, where the new config file will be found.
#
...
if [[ "$fpath" = "" ]]; then
  ...
  fpath="$HOME/dev/$proj/admin.conf"
fi
